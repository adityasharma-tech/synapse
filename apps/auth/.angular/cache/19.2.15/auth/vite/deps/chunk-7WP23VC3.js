...
var Location = class _Location {
    ...
    isCurrentPathEqualTo(path, query = "") {
        return (
            this.path() == this.normalize(path + normalizeQueryParams(query))
        );
    }
    /**
    ...
    switch (name) {
        ...
        case 0:
            ...
            if (extended) {
                ...
                const dayPeriods = getLocaleExtraDayPeriods(
                    locale,
                    form,
                    width
                );
                const index = rules.findIndex((rule) => {
                    if (Array.isArray(rule)) {
                        const [from2, to] = rule;
                        const afterFrom =
                            currentHours >= from2.hours &&
                            currentMinutes >= from2.minutes;
                        const beforeTo =
...
function formatNumberToLocaleString(
    ...
    if (!isFinite(value)) {
        ...
        if (decimals.length) {
            formattedText +=
                getLocaleNumberSymbol(locale, decimalSymbol) +
                decimals.join("");
        }
        if (exponent) {
            formattedText +=
...
function processConfig(config) {
    ...
    if (config.breakpoints) {
        sortedBreakpoints.breakpoints = config.breakpoints.sort(
            (a, b) => a - b
        );
    }
...
function assertValidNgSrcset(dir, value) {
    ...
    const isValidWidthDescriptor =
        VALID_WIDTH_DESCRIPTOR_SRCSET.test(stringVal);
    ...
    if (elements) {
        for (const styleElement of elements) {
            ...
            if (styleElement instanceof HTMLLinkElement) {
                external.set(
                    styleElement.href.slice(
                        styleElement.href.lastIndexOf("/") + 1
                    ),
                    {
                        usage: 0,
...
var DomEventsPlugin = class _DomEventsPlugin extends EventManagerPlugin {
    ...
    addEventListener(element, eventName, handler, options) {
        ...
        return () =>
            this.removeEventListener(element, eventName, handler, options);
    }
...
var BrowserModule = class _BrowserModule {
    constructor() {
        if (typeof ngDevMode === "undefined" || ngDevMode) {
            const providersAlreadyPresent = inject(
...
@angular/platform-browser/fesm2022/platform-browser.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-7WP23VC3.js.map
