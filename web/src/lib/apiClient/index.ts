...
apiClient.interceptors.response.use(response => Promise.resolve(response), async(error)=>{
...
function verifyEmail(payload: VerifyEmailPayloadT){
    return apiClient.get(`/auth/verify?verificationToken=${payload.verificationToken}`)
}
